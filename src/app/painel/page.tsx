...
export default async function PainelPage() {
  ...
  const series = {
    today: todayHourSeries(orders),
    "7": revenueSeries(orders, 7),
    "30": revenueSeries(orders, 30),
  };

  const rev7 = sumRevenue(orders, subDays(todayStart, 6), todayEnd);
  const rev7Prev = sumRevenue(
    orders,
    subDays(todayStart, 13),
    subDays(todayStart, 6),
  );
  const rev30 = sumRevenue(orders, subDays(todayStart, 29), todayEnd);
  const rev30Prev = sumRevenue(
    orders,
